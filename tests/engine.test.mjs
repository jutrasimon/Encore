...
test('both goals are required; failure offers recovery and successful tours continue past three venues',()=>{
 ...
 assert.equal(g.phase,'reward');assert.equal(g.retry,true);assert.equal(g.history[0].won,false);assert.equal(g.e,0);
