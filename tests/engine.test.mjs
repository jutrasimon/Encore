...
test('exhaust leaves draw pool; inactivity stays and enables feedback next round',()=>{const a=board([[3,'kamikaze'],[4,'amp'],[5,'feedback']]);let r=result(a);assert.equal(r.totals.e,14);assert(a[3].exhausted);assert(a[4].inactive);assert(!draw(a.filter(Boolean),random(1)).some(t=>t?.kind==='kamikaze'));r=resolve(a.filter(Boolean),[null,null,null,null,a[4],a[5],null,null,null]);assert.equal(r.totals.e,9);});
