...
test('successful studio keeps gains, resets exhaustion, permits upgrades beyond +3 and unique IDs',()=>{
 ...
 g=act(g,'a','reward',{action:'upgrade',tileId:'a-0'});assert.equal(g.show,1);assert.equal(g.attempt,1);assert.equal(g.players[0].fans,21);assert.equal(g.players[0].inventory[0].level,4);assert.equal(g.players[0].inventory[1].exhausted,false);
