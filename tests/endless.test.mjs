...
test('passing the studio advances only when both players have chosen',()=>{
 let g=band();g.phase='reward';g=act(g,'a','reward',{action:'skip'});assert.equal(g.show,0);assert.equal(g.phase,'reward');
 g=act(g,'b','reward',{action:'skip'});assert.equal(g.show,1);assert.equal(g.players[0].inventory.length,5);assert.deepEqual(targets(g),{q:140,e:128});
});
test('retry keeps gains, resets exhaustion, permits upgrades beyond +3 and never duplicates instance IDs',()=>{
 ...
 const first=g.players[0].inventory.at(-1).id;g.phase='reward';g.retry=true;g.players[0].inventory[0].level=3;g.players[0].inventory[1].exhausted=true;g.players[0].fans=21;
 g=act(g,'a','reward',{action:'upgrade',tileId:'a-0'});assert.equal(g.show,0);assert.equal(g.attempt,1);assert.equal(g.players[0].fans,21);assert.equal(g.players[0].inventory[0].level,4);assert.equal(g.players[0].inventory[1].exhausted,false);
...
test('migration locks existing goals until the next show and resumes previously ended tours',()=>{
 ...
 migrated.phase='reward';const next=act(migrated,'a','reward',{action:'skip'});assert.deepEqual(targets(next),{q:70,e:64});
 const old={...g,phase:'won',show:2,round:5};assert.equal(normalizeGame(old).phase,'reward');assert.equal(normalizeGame({...old,phase:'lost'}).retry,true);
});
