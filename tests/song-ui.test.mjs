...
for(const count of [1,2])for(const win of [true,false])test(`verdict uses actual ${count}-player outcome after five songs (${win})`,()=>{
 ...
 assert.equal(JSON.stringify(g),before);assert.equal((html.match(/class="action-button primary"/g)||[]).length,1);assert.match(html,/data-action="result-detail"/);assert.ok(!html.includes('grid-wrap'));
