...
export const CLASS_ART={
 'guitarist-singer':{portrait:ROOT+'characters/guitariste-chanteur/portrait.png',poses:ROOT+'characters/guitariste-chanteur/poses-sheet.png',microphone:ROOT+'characters/guitariste-chanteur/microphone.png'}
};
...
export class ShowVisual{
 ...
 update(canvas,state,now=performance.now()){
  ...
  if(key!==this.key){
   ...
   Promise.all([texture(showAsset(state.show,'back'),false),...['background','foreground','crowd-sheet'].map(n=>texture(showAsset(state.show,n))),texture(art.poses),texture(art.microphone),texture(ROOT+'shared/crowd-expression-sheet.png')]).then(assets=>{
