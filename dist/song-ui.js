...
import {showVisualMarkup} from './show-art.js?v=0.10.6';
...
export function songCounter(g,{upcoming=false}={}){const n=showInfo(g.show).rounds,next=upcoming&&g.round>0&&g.round<n&&!finishedShow(g),round=g.round+(next?1:0);return `<div class="round song-counter ${lastSong(g)?'song-final':''}" data-song="${g.show}:${round}" aria-label="${lastSong(g)?'Dernière chanson. ':''}Chanson ${round} sur ${n}">${next?'<span class="counter-label">PROCHAINE CHANSON</span>':'CHANSON'} <b>${round}</b><span>/ ${n}</span></div>`;}
