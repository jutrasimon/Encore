...
function draftView(){const p=me();return `<section class="song-reward-screen"><button class="back" data-action="back">← REVOIR LE PLATEAU</button><span class="tape">CHANSON ${game.round} TERMINÉE</span><h1>UNE TUILE<br><span>POUR LA SUITE.</span></h1><div class="song-reward-cards">${p.songOffers.map(kind=>`<article class="song-reward-card type-${type({kind})}"><span class="reward-art">${sticker(kind)}</span><div class="reward-description"><strong>${esc(TILES[kind].name)}</strong>${inventoryEffect({kind,level:0})}</div><button class="primary" data-action="choose-song" data-kind="${kind}" ${busy||mode==='multi'&&!connected?'disabled':''}>CHOISIR</button><button class="reward-info" data-action="inspect-offer" data-kind="${kind}" aria-label="Détails de ${esc(TILES[kind].name)}">?</button></article>`).join('')}</div><button class="secondary" data-action="inventory">INVENTAIRE ET FOCUS</button></section>`;}
