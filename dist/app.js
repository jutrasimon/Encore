...
function musicianStatus(p){
 ...
 if(animating)return 'SUR SCÈNE';
 if(game.phase==='draft')return p.drafted?'CHOIX FAIT':'CHOISIT SA TUILE';
 if(game.phase==='reward')return p.rewarded?'PRÊT':'EN COULISSES';
 return p.ready?'PRÊT !':'SE PRÉPARE';
}
function lobbyView(){
...
function gameView(){if(mode==='multi'&&game.phase==='lobby')return lobbyView();const p=me();if(!p)return '<p>Connexion…</p>';return `<div class="show-top"><button class="show-name" data-action="show-details" aria-label="Détails du show ${esc(SHOWS[game.show].name)}"><small>SHOW ${game.show+1}/${SHOWS.length} ${icon('info')}</small><strong>${esc(SHOWS[game.show].name)}</strong></button><div class="round">CHANSON <b>${game.round}</b><span>/ ${SHOWS[game.show].rounds}</span></div></div>${meters()}${grid()}<div class="readout" aria-live="polite">${animating?'Pige des tuiles…':mode==='multi'&&!connected?'<strong>RECONNEXION…</strong>':p.last?`<span>DERNIÈRE CHANSON</span> ${points(p.last)}`:'Prêt pour la première chanson'}</div>${players()}<div class="action-slot">${animating?'<button class="primary" disabled>CHANSON EN COURS…</button>':game.phase==='show'||game.phase==='draft'?phaseAction():game.phase==='lobby'?'<button class="primary" data-action="show-details">PRÉPARER LE SHOW</button>':'<button class="primary" data-action="result">RÉSULTAT DU SHOW</button>'}</div>`;}
...
function phaseAction(){const p=me();if(game.phase==='lobby')return `<div class="lobby"><h2>${game.players.length===2?'Le band est là.':'La balance est prête.'}</h2><p>${mode==='multi'?'Partage le lien avant de lancer la tournée.':'Trois salles. Un inventaire. À toi de jouer.'}</p>${mode==='multi'?'<button class="secondary" data-action="invite">COPIER LE LIEN D’INVITATION</button>':''}${game.players[0].id===myId?`<button class="primary" data-action="start" ${busy||mode==='multi'&&!connected?'disabled':''}>LANCER LA TOURNÉE ${icon('arrow')}</button>`:'<p>Le créateur lance la tournée.</p>'}</div>`;
 if(game.phase==='draft')return `<button class="primary" data-action="draft" ${p.drafted||busy?'disabled':''}>${p.drafted?'TON PARTENAIRE CHOISIT…':'CHOISIR UNE TUILE'} ${icon('arrow')}</button>`;
...
async function checkServer(){
 ...
 try{const r=await fetch(endpoint+'/health',{signal:AbortSignal.timeout(12000)});const d=await r.json();networkReady=!!(r.ok&&d.ok&&d.protocol===2&&d.rules===2);}
