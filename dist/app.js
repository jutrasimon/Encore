...
function mount(content,preserveDialog=false){
 ...
 juice.release();mountScreen(app,shell(content),preserveDialog);localization.refresh();fitBoard();fitDialogs();screenMotion.settle(before);syncShowVisual();songEffects.sync(game);
}
...
function resolutionView(){
 ...
 return `<div class="resolution-shade" aria-hidden="true"></div><div class="resolution-top"><div><small>SUR SCÈNE · ${g.index+1}/${cinematic.plan.groups.length}</small><strong><span translate="no">${esc(g.player.name)}</span></strong></div>${songCounter(game)}<button class="live-sound" data-action="sound" aria-pressed="${sound}" aria-label="${sound?'Couper le son':'Activer le son'}">${icon('sound')} ${sound?'SON':'MUET'}</button></div>${meters()}${grid()}<div class="resolution-tally">${[['q','star','QUALITÉ'],['e','bolt','ÉNERGIE']].map(([k,i,label])=>`<div class="charge-counter charge-${k}" data-charge="${k}"><span>${icon(i)} ${label}</span><b>0</b><i></i></div>`).join('')}</div><div class="resolution-caption" role="status"><strong>FAIS DU BRUIT !</strong><span class="resolution-fans"></span></div>${showVisualMarkup()}<div class="resolution-pop" aria-hidden="true"><strong><span translate="no">${esc(g.player.name)}</span></strong></div><div class="resource-flights" aria-hidden="true"></div>`;
}
function players(){return `<div class="bandmates">${game.players.map(p=>`<button class="bandmate ${p.id===myId?'current-musician':''}" data-action="profile" data-id="${esc(p.id)}" aria-label="${esc(p.name)}" data-player-label="true"><span class="portrait">${classPortrait(p)}</span><span class="musician-mini"><strong><span translate="no">${esc(p.name)}</span></strong><span>${icon('choir')}${p.fans} FANS</span>${mode==='multi'?`<small class="musician-status">${musicianStatus(p)}</small>`:''}</span></button>`).join('')}<button class="band-total" data-action="stats"><b>${icon('choir')}${game.players.reduce((n,p)=>n+p.fans,0)}</b><span>FANS DU BAND</span></button></div>`;}
...
app.addEventListener('change',e=>{if(e.target.id==='language'){setLanguage(e.target.value);save('encore.language',getLanguage());resolutionAudio.cancelSpeech();render();localization.refresh();}});
