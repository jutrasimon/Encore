import {TILES,showInfo,newGame,player,command,targets,adjacent,normalizeGame,focusCapacity,ROLES} from './engine.js?v=0.7.2';
import {tileCard,tileDetails} from './tile-ui.js?v=0.7.2';
import {inventoryMarkup} from './inventory-ui.js?v=0.7.2';
...
import {api, BandConnection, credential, inviteCode} from './network.js?v=0.7.2';
...
function shell(content){document.body.classList.toggle('reduced-motion',!motion);return `${header()}<section class="console ${game?'in-game':'at-home'} ${animating?'resolution-mode':''}"><div class="screen-body" data-screen="${view}">${content}</div>${game&&view==='game'&&!animating&&!(mode==='multi'&&game.phase==='lobby')?playAction():''}${game?nav():''}</section><dialog id="details"></dialog>`;}
