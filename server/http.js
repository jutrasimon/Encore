...
export function handler(store) {
 return async request=>{
  ...
  if(path==='health'&&request.method==='GET')return respond({ok:true,protocol:2});
  ...
  try {
    ...
    if(!['hello','sync','start','ready','reward'].includes(msg.type))throw new GameError('Action inconnue.');
